...
public sealed class SkillRepository : ISkillRepository
{
    ...
    public async Task<IEnumerable<Skill>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        return await _context.Skills
            .Where(s => !s.IsDeleted)
            .Include(s => s.SecondarySkills.Where(ss => !ss.IsDeleted))
            .AsNoTracking()
    ...
    public async Task<IEnumerable<Skill>> GetSecondarySkillsAsync(Guid primarySkillId, CancellationToken cancellationToken = default)
    {
        return await _context.Skills
            .Where(s => !s.IsDeleted && s.ParentSkillId == primarySkillId)
            .AsNoTracking()
            .ToListAsync(cancellationToken);
    }

    public async Task<bool> HasActiveSecondariesAsync(Guid primarySkillId, CancellationToken cancellationToken = default)
    {
        return await _context.Skills
            .AnyAsync(s => !s.IsDeleted && s.ParentSkillId == primarySkillId, cancellationToken);
    }
}
